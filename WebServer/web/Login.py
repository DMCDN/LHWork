...
def login():
    if request.method == 'POST':
        ...
        LWork_cursor.execute('SELECT id, IsVerified, V_ExpirationTime, Email FROM Members WHERE UserName = ? AND Pwd = ?', (username, password))
        result = LWork_cursor.fetchone()
        if result:
            UserID, IsVerified, expirationTime, Email = result[0], result[1], result[2], result[3]
            # 尚未驗證
            if not IsVerified:
                # 驗證碼 已過期
...
#    LWork_cursor.execute('SELECT id FROM Members WHERE UserName = ? AND Pwd = ?', (username, password))
#    result = LWork_cursor.fetchone()
#    if result:
...
def Register():
    if request.method == 'POST':
        ...
        LWork_cursor.execute('INSERT INTO Members (UserName, Pwd, Email, VerificationCode, V_ExpirationTime, IsVerified) VALUES (?, ?, ?, ?, ?, ?)',
                            (username, password, email, verification_code, expirationTime_epoch, 0))
        LWork_conn.commit()
        return '註冊成功，請至信箱驗證'
